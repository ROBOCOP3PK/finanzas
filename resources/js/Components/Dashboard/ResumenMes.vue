<template>
    <Card title="Resumen del Mes">
        <div class="space-y-3">
            ...
            <div class="flex justify-between items-center">
                <span class="text-gray-600 dark:text-gray-400">Gastos {{ nombreUsuario2 }} (100%)</span>
                <span class="font-medium text-red-500">{{ formatCurrency(gastosPareja) }}</span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-gray-600 dark:text-gray-400">Compartidos ({{ porcentajePersona2 }}% {{ nombreUsuario2 }})</span>
...
const nombreUsuario2 = computed(() => configStore.nombre_persona_2 || 'Usuario 2');
